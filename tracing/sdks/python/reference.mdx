...
`redaction` uses snake_case keys in Python. Redaction applies only to
...
| `sensitive_keys` | `list[str]` | built-in list | Force-redact matching keys such as `email`, `api_key`, `authorization`, and `cookie` inside IO payloads |
| `custom_patterns` | `list[Pattern[str] \| str]` | `[]` | Additional regex patterns to redact inside IO payloads |
...
| `ClaudeAgentIntegration`   | `"claude_agent"`  | Claude Agent SDK turns        |
