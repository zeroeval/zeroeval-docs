...
The Python SDK automatically instruments popular frameworks through monkey-patching:
...
trace_id = ze.get_current_trace_id()

# Get the current session ID
session_id = ze.get_current_session_id()
```
...
- Learn about [Sessions](/tracing/sdks/python/sessions) for grouping related spans
- Explore [Tagging](/tracing/sdks/python/tagging) for better organization
- Implement [Signals](/tracing/sdks/python/signals) to capture user feedback
- Check out [Advanced Configuration](/tracing/sdks/python/configuration) options 