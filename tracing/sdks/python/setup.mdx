...
The SDK automatically propagates span context across `threading.Thread` and `ThreadPoolExecutor` workers. Spans created in child threads nest under the parent with no extra code:
...
def process_chunk(chunk_id):
    with ze.span("process_chunk", tags={"chunk": chunk_id}):
        # This span is a child of "parallel_pipeline"
        return analyze(chunk_id)

with ze.span("parallel_pipeline"):
    threads = [threading.Thread(target=process_chunk, args=(i,)) for i in range(4)]
    for t in threads:
        t.start()
    for t in threads:
        t.join()
...
## Multi-Process Apps (Celery, Workers)

`ze.init()` is idempotent — calling it multiple times is safe. Place it inside the function that runs in the process you want to trace, not at module scope in shared files:
...
def my_task(data):
    import zeroeval as ze
    ze.init()  # safe to call every invocation

    with ze.span("my_task"):
        return process(data)
```

<Warning>
  Avoid calling `ze.init()` at module scope in files imported by multiple
  processes (e.g. both an API server and a Celery worker). This can cause
  duplicate spans from the process that shouldn't be tracing.
</Warning>
