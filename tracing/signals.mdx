...
• **Send signals as they happen** – latency isn't an issue, we process them async  
• **Re-use the same `name` consistently** – that's how we aggregate and analyze  
• **One target can have multiple signals** (different names)  
• **Overwrite behavior** – if you POST the same `(target_id, name)` twice, we'll overwrite the previous value  
• **Use meaningful names** – `user_satisfaction` is better than `signal1`  
• **Choose appropriate targets** – completion signals for LLM responses, session signals for user behavior
