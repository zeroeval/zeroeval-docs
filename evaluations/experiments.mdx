...
• Use multiple evaluators – they all run in one pass.  
• Pass `raise_on_error=True` to `.run_task()` while debugging.  
• Results stream live to the dashboard; no need to wait synchronously.
