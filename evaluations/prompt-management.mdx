...
p = ze.get_prompt(
    "support-triage",
    version=None,            # int | None - if set, overrides tag
    tag=None,                # str | None - e.g. "production", "staging", "latest"
    fallback=None,           # str | None - local content if fetch fails / 404
    variables=None,          # dict | None - {name: value} for {{name}} templating
    task_name=None,          # str | None - associate content with a tuning task
    render=True,             # bool - render template with variables
    missing="error",        # "error" | "leave" - behavior for missing variables
    use_cache=True,          # bool - in-process TTL cache for server results
    timeout=None,            # float | None - per-request timeout (seconds)
)
...
```python
p = ze.get_prompt("support-triage", tag="staging", fallback="You are a helpful assistant.")
```
...
- Pin to a tag or a specific version:

```python
ze.get_prompt("support-triage", tag="staging")
ze.get_prompt("support-triage", version=12)
```
...
- Required: `ZEROEVAL_API_KEY`.
- Optional:
  - `ZEROEVAL_BASE_URL` or `ZEROEVAL_API_URL`
  - `ZEROEVAL_PROMPT_TAG`
  - `ZEROEVAL_ENV` (controls default tag)
